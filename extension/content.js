...
(function() {
  ...
  // Tags whose content should never be highlighted (Set for O(1) acceptNode checks)
  const SKIP_TAGS = new Set(["SCRIPT","STYLE","TEXTAREA","INPUT","SELECT","NOSCRIPT"]);

  // Groups (block-level elements) processed per async chunk on large pages
  const GROUP_CHUNK_SIZE = 20;

  let globalEnabled = true;
  let highlightGeneration = 0; // incremented each pass; lets orphaned async chunks abort
  ...
  function highlightAll(root) {
    ...
    // Small subtrees (e.g. from MutationObserver on a newly added element):
    // process synchronously — scheduling overhead outweighs the benefit.
    if (groups.length <= GROUP_CHUNK_SIZE) {
      for (const group of groups) highlightGroup(group);
      return;
    }

    // Large pages: yield between chunks so the browser can paint and respond
    // to input. highlightGeneration lets a new pass cancel an in-flight one.
    const gen = ++highlightGeneration;
    function processChunk(i) {
      if (gen !== highlightGeneration) return; // newer pass started; abort
      if (!globalEnabled || !compiledMatcher) return;
      const end = Math.min(i + GROUP_CHUNK_SIZE, groups.length);
      for (let j = i; j < end; j++) highlightGroup(groups[j]);
      if (end < groups.length) setTimeout(() => processChunk(end), 0);
    }
    processChunk(0);
  }
